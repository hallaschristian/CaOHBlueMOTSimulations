...
@everywhere begin
    ...
    prob = remake(prob, tspan=(0, 11e-3/(1/Γ)))
...
writedlm("data/1plus2_diffusions_x.txt", stack(diffusions_over_time[:,1,end,:]'))
writedlm("data/1plus2_diffusions_z.txt", stack(diffusions_over_time[:,2,end,:]'))
...
σs = σ_vs_time(sols[1])
io = open("data/1plus2_size_vs_time.txt", "w") do io
    for σ in σs
        println(io, σ)
    end
end
...
io = open("data/1plus2_density_vs_time.txt", "w") do io

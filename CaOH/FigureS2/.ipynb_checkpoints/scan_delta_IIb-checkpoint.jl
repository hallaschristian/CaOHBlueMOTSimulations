...
writedlm("data/delta_IIb_diffusions_x213.txt", stack(diffusions_over_time[:,1,end,:]'))
writedlm("data/delta_IIb_diffusions_z213.txt", stack(diffusions_over_time[:,2,end,:]'))
...
σs = compute_σ.(sols)
open("data/delta_IIb_sizes213.txt", "w") do io
    for σ in σs
        println(io, σ)
    end
end
...
open("data/delta_IIb_densities213.txt", "w") do io

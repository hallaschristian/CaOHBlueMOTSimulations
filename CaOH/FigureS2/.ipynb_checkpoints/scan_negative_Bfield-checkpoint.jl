...
writedlm("data/negative_Bfield_diffusions_x7.txt", stack(diffusions_over_time[:,1,end,:]'))
writedlm("data/negative_Bfield_diffusions_z7.txt", stack(diffusions_over_time[:,2,end,:]'))
...
σs = compute_σ.(sols)
open("data/negative_Bfield_sizes7.txt", "w") do io
    for σ in σs
        println(io, σ)
    end
end
...
open("data/negative_Bfield_densities7.txt", "w") do io

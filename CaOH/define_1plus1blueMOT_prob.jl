...
p_1plus1 = initialize_prob(Float64, energies, freqs, sats, pols, k_relative, beam_radius, d, m/(ħ*k^2/Γ), Γ, k, sim_params, update_p_1plus1!, add_terms_dψ!, 1:12, 13:16)
...
prob_1plus1 = ODEProblem(ψ_update!, p_1plus1.u0, t_span, p_1plus1; kwargs...)

# PROBLEM TO COMPUTE DIFFUSION #
p_1plus1_diffusion = initialize_prob(Float64, energies, freqs, sats, pols, k_relative, beam_radius, d, m/(ħ*k^2/Γ), Γ, k, sim_params, update_p_1plus1_diffusion!, add_terms_dψ!, 1:12, 13:16)
cb0 = DiscreteCallback(condition_discrete, stochastic_collapse_nokick!, save_positions=(false,false))
cbs_diffusion = CallbackSet(cb0)
kwargs = (alg=DP5(), reltol=1e-4, abstol=1e-5, saveat=1000, maxiters=100000000, callback=cbs_diffusion)
prob_1plus1_diffusion = ODEProblem(ψ_update_ballistic!, p_1plus1_diffusion.u0, t_span, p_1plus1_diffusion; kwargs...)

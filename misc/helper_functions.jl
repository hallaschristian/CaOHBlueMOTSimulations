...
function compute_T(sols)
    vxs = [vx(sol.u[end]) for sol ∈ sols if survived(sol)]
    vys = [vy(sol.u[end]) for sol ∈ sols if survived(sol)]
    vzs = [vz(sol.u[end]) for sol ∈ sols if survived(sol)]
    Tx = T_fit_1D(vxs)
    Ty = T_fit_1D(vys)
    Tz = T_fit_1D(vzs)
    return (Tx * Ty * Tz)^(1/3)
...
using ProgressMeter
function distributed_solve(n_trajectories, prob, prob_func, scan_func=scan_nothing, scan_values=[0])

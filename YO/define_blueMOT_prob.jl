### SIMULATION PROBLEMS DEFINITION: 1+2 BLUE MOT ###

using DifferentialEquations

# PROBLEM TO CALCULATE TRAJECTORIES #
t_start = 0.0
t_end   = 60e-3
t_span  = (t_start, t_end) ./ (1/Γ)

coupling_idxs = [[5:16,17:20],[1:4,5:16]]
p = initialize_prob(Float64, energies, freqs, sats, pols, ks_relative, beam_radius, offsets, d, m/(ħ*k^2/Γ), Γ, k, sim_params, update_p!, add_terms_dψ!, 1:16, 17:20, coupling_idxs)

function diffusion_kick(integrator)
    p = integrator.p
    n_states = p.n_states
    n_e = p.n_e
    @inbounds @fastmath for i ∈ 1:3
        kick = sqrt(2p.diffusion_constant[i] * p.sim_params.dt_diffusion) / p.m
        integrator.u[2n_states + n_e + 3 + i] += rand((-1,1)) * kick
    end
    return nothing
end

diffusion_times = (0:1e-7:100e-3) ./ (1/Γ)
cb_diffusion = PresetTimeCallback(diffusion_times, diffusion_kick, save_positions=(false,false))
cb1 = DiscreteCallback(condition_discrete, stochastic_collapse_nokick!, save_positions=(false,false))
cb2 = DiscreteCallback(terminate_condition, terminate!)
cbs = CallbackSet(cb1, cb2, cb_diffusion)
kwargs = (alg=DP5(), reltol=1e-4, abstol=1e-5, saveat=1000, maxiters=1000000000, callback=cbs)

prob = ODEProblem(ψ_update_multiple!, p.u0, t_span, p; kwargs...)

# PROBLEM TO COMPUTE DIFFUSION #
p_diffusion = initialize_prob(Float64, energies, freqs, sats, pols, ks_relative, beam_radius, offsets, d, m/(ħ*k^2/Γ), Γ, k, sim_params, update_p_diffusion!, add_terms_dψ!, 1:16, 17:20, coupling_idxs)
# cbs_diffusion = CallbackSet(cb1, cb_diffusion)
cbs_diffusion = CallbackSet(cb1)
kwargs = (alg=DP5(), reltol=1e-4, abstol=1e-5, saveat=1000, maxiters=1000000000, callback=cbs_diffusion)
prob_diffusion = ODEProblem(ψ_update_multiple_ballistic!, p_diffusion.u0, t_span, p_diffusion; kwargs...)

function update_phases!(prob)
    for i in eachindex(prob.p.coupling_idxs)
        for k ∈ 1:3
            for f ∈ axes(prob.p.ϵs[i],2)
                ϕ1 = exp(im * 2π * rand())
                ϕ2 = exp(im * 2π * rand())
                for q ∈ axes(prob.p.ϵs[i],3)
                    prob.p.ϵs_with_phases[i][k,f,q] = prob.p.ϵs[i][k,f,q] * ϕ1
                    prob.p.ϵs_with_phases[i][k+3,f,q] = prob.p.ϵs[i][k+3,f,q] * ϕ2
                end
            end
        end
    end
    return nothing
end

function prob_func_YO!(prob)
    reset_prob!(prob)
    prob.p.sim_params.photon_budget = 10000000
    update_initial_position!(prob, sample_position(prob.p.sim_params) ./ (1/k))
    update_initial_velocity!(prob, sample_velocity(prob.p.sim_params) ./ (Γ/k))
    update_phases!(prob)
    return nothing
end

function prob_func_diffusion_YO!(prob)
    prob_func_YO!(prob)
    prob.p.sim_params.Bx = +x(prob.u0) * prob.p.sim_params.B_grad_end * 1e2 / 2
    prob.p.sim_params.By = +y(prob.u0) * prob.p.sim_params.B_grad_end * 1e2 / 2
    prob.p.sim_params.Bz = -z(prob.u0) * prob.p.sim_params.B_grad_end * 1e2
    return nothing
end

x(u) = real(u[40+4+1]) * (1/k)
y(u) = real(u[40+4+2]) * (1/k)
z(u) = real(u[40+4+3]) * (1/k)
vx(u) = real(u[40+4+4]) * (Γ/k)
vy(u) = real(u[40+4+5]) * (Γ/k)
vz(u) = real(u[40+4+6]) * (Γ/k)

# ### SIMULATION PROBLEMS DEFINITION: 1+2 BLUE MOT ###

# using DifferentialEquations

# # PROBLEM TO CALCULATE TRAJECTORIES #
# t_start = 0.0
# t_end   = 30e-3
# t_span  = (t_start, t_end) ./ (1/Γ)

# p = initialize_prob(Float64, energies, freqs, sats, pols, k_relative, beam_radius, offsets, d, m/(ħ*k^2/Γ), Γ, k, sim_params, update_p!, add_terms_dψ!, 1:12, 13:16)

# function diffusion_kick(integrator)
#     p = integrator.p
#     n_states = p.n_states
#     n_e = p.n_e
#     @inbounds @fastmath for i ∈ 1:3
#         kick = sqrt(2p.diffusion_constant[i] * p.sim_params.dt_diffusion) / p.m
#         integrator.u[2n_states + n_e + 3 + i] += rand((-1,1)) * kick
#     end
#     return nothing
# end

# diffusion_times = (0:1e-7:t_end) ./ (1/Γ)
# cb_diffusion = PresetTimeCallback(diffusion_times, diffusion_kick, save_positions=(false,false))
# cb1 = DiscreteCallback(condition_discrete, stochastic_collapse_nokick!, save_positions=(false,false))
# cb2 = DiscreteCallback(terminate_condition, terminate!)
# cbs = CallbackSet(cb1, cb2, cb_diffusion)
# kwargs = (alg=DP5(), reltol=1e-4, abstol=1e-5, saveat=1000, maxiters=1000000000, callback=cbs)

# prob = ODEProblem(ψ_update!, p.u0, t_span, p; kwargs...)

# # PROBLEM TO COMPUTE DIFFUSION #
# p_diffusion = initialize_prob(Float64, energies, freqs, sats, pols, k_relative, beam_radius, offsets, d, m/(ħ*k^2/Γ), Γ, k, sim_params, update_p_diffusion!, add_terms_dψ!, 1:12, 13:16)
# # cbs_diffusion = CallbackSet(cb1, cb_diffusion)
# cbs_diffusion = CallbackSet(cb1)
# kwargs = (alg=DP5(), reltol=1e-4, abstol=1e-5, saveat=1000, maxiters=1000000000, callback=cbs_diffusion)
# prob_diffusion = ODEProblem(ψ_update_ballistic!, p_diffusion.u0, t_span, p_diffusion; kwargs...)

# function update_phases!(prob)
#     for k ∈ 1:3
#         for f ∈ axes(prob.p.ϵs,2)
#             ϕ1 = exp(im * 2π * rand())
#             ϕ2 = exp(im * 2π * rand())
#             for q ∈ axes(prob.p.ϵs,3)
#                 prob.p.ϵs_with_phases[k,f,q] = prob.p.ϵs[k,f,q] * ϕ1
#                 prob.p.ϵs_with_phases[k+3,f,q] = prob.p.ϵs[k+3,f,q] * ϕ2
#             end
#         end
#     end
#     return nothing
# end

# function prob_func_YO!(prob)
#     reset_prob!(prob)
#     prob.p.sim_params.photon_budget = 10000000
#     update_initial_position!(prob, sample_position(prob.p.sim_params) ./ (1/k))
#     update_initial_velocity!(prob, sample_velocity(prob.p.sim_params) ./ (Γ/k))
#     update_phases!(prob)
#     return nothing
# end

# function prob_func_diffusion_YO!(prob)
#     prob_func_YO!(prob)
#     prob.p.sim_params.Bx = +x(prob.u0) * prob.p.sim_params.B_grad * 1e2 / 2
#     prob.p.sim_params.By = +y(prob.u0) * prob.p.sim_params.B_grad * 1e2 / 2
#     prob.p.sim_params.Bz = -z(prob.u0) * prob.p.sim_params.B_grad * 1e2
#     return nothing
# end

# x(u) = real(u[32+4+1]) * (1/k)
# y(u) = real(u[32+4+2]) * (1/k)
# z(u) = real(u[32+4+3]) * (1/k)
# vx(u) = real(u[32+4+4]) * (Γ/k)
# vy(u) = real(u[32+4+5]) * (Γ/k)
# vz(u) = real(u[32+4+6]) * (Γ/k)